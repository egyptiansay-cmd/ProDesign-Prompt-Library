...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  // Changed initial state from ['Print'] to [] to start collapsed
    ...
    return (
      <div key={cat.id} className="mb-1">
        <button
          onClick={() => {
            if (hasSubs) {
              // Toggle expansion and select parent
              toggleExpand(cat.id);
        ...
        >
          ...
          {/* Active Indicator Line (Only for selected items or parents of selected) */}
